...
{
    public sealed class NacosReverseProxyDocumentFilter : IDocumentFilter
    {
        ...
        private readonly IProxyStateLookup _proxyConfigProvider;

        public NacosReverseProxyDocumentFilter(
            IHttpClientFactory httpClientFactory,
            IOptionsMonitor<ReverseProxyDocumentFilterConfig> configOptions,
            IEnumerable<ITransformFactory> factories,IProxyStateLookup proxyConfigProvider)
        {
        ...
        )
        {
            ...
            var proxyConfig=_proxyConfigProvider.GetClusters().ToList();
            ...
            else
            {
                ...
                {
                    var targetCluster=proxyConfig.FirstOrDefault(it => it.ClusterId == cluster.Key);
                    
                    foreach (var destination in cluster.Value.Destinations)
                    {
                        var targetDestination=targetCluster!.Destinations!.FirstOrDefault(it => it.Key.Contains(destination.Key));
                        destination.Value.Address = targetDestination.Value.Model.Config.Address;
                    }
                }
            }
            ...
            foreach (var clusterKeyValuePair in clusters)
            {
                ...
                foreach (var destination in cluster.Destinations)
                {
                    ...
                    foreach (var swagger in destination.Value.Swaggers)
                    {
                        ...
                        {
                            var url=$"{destination.Value.Address}{swaggerPath}";

                            if(destination.Value.Address.EndsWith("/")&&swaggerPath.StartsWith("/"))
                                url= $"{destination.Value.Address}{swaggerPath.Substring(1)}";
