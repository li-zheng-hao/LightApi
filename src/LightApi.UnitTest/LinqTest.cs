...
public class LinqTest
{
    ...
    public void DynamicWhere(string opType, string property, object? value, int resultCount)
    {
        ...
        {
            queryJsonBody =
                "{\"Property\":\"" + property + "\",\"OpType\":\"" + opType + "\",\"Value\":null}";
        }
        else
        {
            queryJsonBody =
    ...
    [InlineData("In", "A", "[\"abc\"]", 1)]
    [InlineData("In", "F", "[\"A\"]", 1)]
    public void DynamicWhereForSystemTextJson(
        string opType,
        string property,
        object? value,
        int resultCount
    )
    {
        ...
        var query = System.Text.Json.JsonSerializer.Deserialize<DynamicQueryDto>(
            queryJsonBody,
            serializerOptions
        );
        var data = GetTestData();
    ...
    [Fact]
    public void DynamicOrderIgnoreCase()
    {
        var data = GetTestData().AsQueryable();

        // 测试升序排序
        var resultAsc = data.DynamicOrder("a", false).ToList();
        Assert.Equal("abc", resultAsc[0].A); // 确保第一个元素是 "abc"

        // 测试降序排序
        var resultDesc = data.DynamicOrder("a", true).ToList();
        Assert.Equal("ghi", resultDesc[0].A); // 确保第一个元素是 "ghi"

        // 测试忽略大小写
        var resultIgnoreCase = data.DynamicOrder("A", false).ToList();
        Assert.Equal("abc", resultIgnoreCase[0].A); // 确保第一个元素是 "abc"
    }
