...
internal static class TypeExtension
{
    ...
    public static bool IsVisible(this MethodBase methodBase) =>
        methodBase.IsPublic || methodBase.IsFamily || methodBase.IsFamilyOrAssembly;

    public static Func<T, object> GetValueGetter<T>(this PropertyInfo propertyInfo)
    {
        return StrongTypedDictionary<T>.PropertyValueGetters.GetOrAdd(
            propertyInfo,
            prop =>
            {
                if (!prop.CanRead)
    ...
    {
        return ReflectionDictionary.PropertyValueGetters.GetOrAdd(
            propertyInfo,
            prop =>
            {
                if (!prop.CanRead)
    ...
    {
        return ReflectionDictionary.PropertyValueSetters.GetOrAdd(
            propertyInfo,
            prop =>
            {
                if (!prop.CanWrite)
    ...
    public static Action<T, object> GetValueSetter<T>([NotNull] this PropertyInfo @this)
        where T : class
    {
        return StrongTypedDictionary<T>.PropertyValueSetters.GetOrAdd(
            @this,
            prop =>
            {
                if (!prop.CanWrite)
                ...
                return (Action<T, object>)
                    Expression.Lambda(setterCall, instance, argument).Compile();
            }
        );
    }
