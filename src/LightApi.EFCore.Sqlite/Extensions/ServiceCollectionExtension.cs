...
using LightApi.EFCore.Sqlite.Transaction;
...
public static class ServiceCollectionExtension
{
    ...
    /// <returns></returns>
    public static IServiceCollection AddInfrastructureEfCoreSqlite<TAppContext>(
        ...
        where TAppContext : AppDbContext
    {
        ...
        if (isMasterDbContext)
        {
            ...
            services.TryAddScoped<IUnitOfWork, SqliteUnitOfWork<TAppContext>>();
        }
        services.TryAddScoped<SqliteUnitOfWork<TAppContext>>();

        Db.AddDbContextModelMap(typeof(TAppContext), entityInfoType);

        Db.AddDbContextUnitOfWorkMap(typeof(TAppContext), typeof(SqliteUnitOfWork<TAppContext>));
