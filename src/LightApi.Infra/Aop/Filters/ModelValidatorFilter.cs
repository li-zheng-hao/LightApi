...
public class ModelValidatorFilter : IResultFilter
{
    ...
    {
        var options = actionContext.HttpContext.RequestServices.GetService<
        ...
        if (!modelState.IsValid)
        {
            ...
            if (errorMsg.IsNullOrWhiteSpace())
                errorMsg = options!.Value.DefaultModelValidateErrorMessage;
    ...
    private string GenerateFriendlyMessage(string error)
    {
        try
        {
            ...
            {
                var inputValue = error
                    .Substring("Could not convert string to integer ".Length)
                    .Split('.')
                    .FirstOrDefault();
            ...
            {
                var inputValue = error
                    .Substring("Could not convert string to decimal: ".Length)
                    .Split('.')
                    .FirstOrDefault();
                if (inputValue != null)
