...
public class AutofacModuleRegister : Module
{
    ...
    {
        var assemblies = AppDomain.CurrentDomain.GetAssemblies().Where(it => _dllPrefixes.Any(df=>it.FullName?.StartsWith(df)==true)).ToList();

        assemblies.ForEach(it =>
        {
            var transient = it.GetExportedTypes()
                .Where(type => type.IsClass && type.IsAssignableTo(typeof(ITransientDependency))).ToArray();
            ...
            var scoped = it.GetExportedTypes()
                .Where(type => type.IsClass && type.IsAssignableTo(typeof(IScopedDependency))).ToArray();
            ...
            var singleton = it.GetExportedTypes()
                .Where(type => type.IsClass && type.IsAssignableTo(typeof(ISingletonDependency))).ToArray();
